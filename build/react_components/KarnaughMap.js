...
var KarnaughCell = React.createClass({displayName: 'KarnaughCell',
  ...
  render: function() {
    ...
    return (
      React.DOM.div({ref: "div", 
        ...
        React.DOM.div({className: "React-KarnaughCell-Inner"},  STATE_DISPLAY[ props.state] )
      )
    );
  }
});

var KarnaughMap = React.createClass({displayName: 'KarnaughMap',
  ...
  render: function() {
    ...
    if( numVars == 2 ) {
      // first row, m0 m2
      rows.push(React.DOM.div({className: "React-KarnaughMap-Row"}, 
        [ cells[ 0 ], cells[ 2 ]]
      ));
    ...
    return (React.DOM.div({ref: "wrapper", 
                 className: "React-KarnaughMap", 
                 onKeyDown:  this.onKeyDown, 
                 onKeyUp:  this.onKeyUp}, 
      rows 
    )); // React will complain here about lack of keys for this array -- fix later
  }
});
